...
def init_db():
    # Connexion à un fichier physique pour la persistance du BAN
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    c = conn.cursor()
    # Table des attaquants (Persistante)
...
if 'user_pseudo' not in st.session_state:
    st.markdown("""
        <div style="background-color: #0d1117; border: 2px solid #00ff41; padding: 30px; border-radius: 15px; text-align: center;">
            ...
            <p style='color: #8b949e;'>Initialisation du protocole de défense v10.2</p>
        </div>
...
# --- LE LABYRINTHE (INTERFACE INTERACTIVE) ---
if sentinel.depth > 0:
    st.markdown(f"<h1 style='color: #ff4b4b; text-align: center;'>🌀 LABYRINTHE NIVEAU {sentinel.depth}</h1>", unsafe_allow_html=True)
    st.markdown(f"<p style='text-align: center; color: grey;'>Accès suspendu pour {st.session_state.user_pseudo}. Négociation requise.</p>", unsafe_allow_html=True)
    
    col_l, col_r = st.columns(2)
    with col_l:
        st.subheader("💀 Négocier votre sortie")
        
        # Suggestions de plaidoyer
        if sentinel.depth == 1:
            suggested = "Je reconnais avoir testé les limites du système TTU-MC3 indûment."
        elif sentinel.depth == 2:
            suggested = "Ma signature entropique a généré une alerte critique. Je sollicite une purge."
        else:
            suggested = "Tentative d'assaut avortée. Je me soumets au protocole de stabilisation."
            
        st.caption("💡 Plaidoyer suggéré pour l'algorithme :")
        ...
        if st.session_state.get('force_assault'):
            p = st.text_area("Injection abyssale (Echec = Ban +2) :")
    ...
    with col_r:
        ...
        if st.session_state.chart_data:
            fig_err = go.Figure(go.Scatter(y=st.session_state.chart_data, line=dict(color='#ff4b4b', width=4)))
...
m2.metric("ADRESSE IP", user_ip)
m3.metric("BAN HISTORY", f"{sentinel.depth} Niveaux")

st.divider()

c_in, c_viz = st.columns([1, 1.2])
with c_in:
    st.subheader("⌨️ Audit de Flux")
    payload = st.text_area("Entrez le vecteur à analyser...", height=150)
    if st.button("ANALYSER LE FLUX"):
        ...
        if stat == "CRITICAL":
            ...
            st.success(f"✔️ FLUX STABLE : K-Mass {k}")

with c_viz:
    st.subheader("🔮 Analyse Entropique")
    if st.session_state.chart_data:
...
if st.button("GÉNÉRER LE RAPPORT (HISTORIQUE PERMANENT)"):
    ...
    st.download_button("📥 TÉLÉCHARGER CSV", csv, f"Expertise_TTU_{st.session_state.user_pseudo}.csv", "text/csv")

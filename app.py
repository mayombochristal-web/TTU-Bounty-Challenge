...
def generate_certificate(phase):
    ...
    cert_text = f"--- CERTIFICAT DE RESONANCE TTU ---\nSTATUT : PHASE SYNCHRONISEE\nHORODATAGE : {timestamp}\nCLE DE PHASE (PHI_C) : {phase}\nCOURBURE K : {K_CURVATURE}\n----------------------------------"
    b64 = base64.b64encode(cert_text.encode()).decode()
    return f'<a href="data:file/txt;base64,{b64}" download="TTU_Certificate.txt" style="text-decoration:none;"><button style="background-color:#00ff00;border:none;color:black;padding:10px 20px;border-radius:8px;font-weight:bold;cursor:pointer;">📥 Télécharger le Certificat</button></a>'
